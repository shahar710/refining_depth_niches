...
```{r}
bruvs_raw_fishbase=bruvs_raw_depth %>% 
  left_join(fishbase_depths, by="species") %>% 
...
```{r}
#Mediterranean Sea
med_p=ggplot(bruvs_raw_fishbase %>% 
         filter(either_min_max%in%"yes",
                sea%in%"med"))+
  geom_linerange(aes(y=reorder(species,max_depth_diff),
                     xmin=min_depth,
                     xmax=max_depth),
                 color="blue",
                 size=2.5)+
...
#Red Sea
red_p=ggplot(bruvs_raw_fishbase %>% 
         filter(either_min_max%in%"yes",
                sea%in%"red"))+
  geom_linerange(aes(y=reorder(species,max_depth_diff),
                     xmin=min_depth,
...
Combine two plots
```{r}
both_seas_raw_p=gridExtra::grid.arrange(med_p, red_p,
                        ncol = 1,layout_matrix = rbind(c(1),
                                                       c(2),
                                                       c(2),
                                                       c(2)))
#ggsave(plot = both_seas_raw_p, filename = "both_seas_raw_p.png",device ="jpeg",height = 25,width = 20, units = "cm",dpi = 600)
```
