...
G_j=tibble(
  ...
  predicted=as.numeric(predict(ehof_red_all$`Gymnothorax johnsoni`,model = "I",newdata = data.frame(x = 0:150))))
#observed
G_j_o=tibble(
...
G_j_p=ggplot(G_j)+
  ...
  geom_point(data=G_j_o %>% 
             ...
             fill="black",
             size=1.5)+
  geom_line(aes(x=x,y=predicted*max(ehof_red_all$`Gymnothorax johnsoni`$y)),
            color="black",
            size=1,
            linejoin="round",
            lineend="round")+
...
T_p=tibble(
  x=as.numeric(seq(0,150,by=1)),
  predicted=as.numeric(predict(ehof_med_all$`Thalassoma pavo`,model = "II",newdata = data.frame(x = 0:150))[,]))
#observed
T_p_o=tibble(
...
T_p_p=ggplot(T_p)+
  ...
  geom_point(data=T_p_o %>% 
             ...
             fill="black",
             size=1.5)+
  geom_line(aes(x=x,y=predicted*max(ehof_med_all$`Thalassoma pavo`$y)),
            color="red",
            size=1,
            linejoin="round",
            lineend="round")+
...
#observed
P_c_o=tibble(
    x=ehof_med_all$`Pagrus caeruleostictus`$x,
    y=ehof_med_all$`Pagrus caeruleostictus`$y)
#plot
P_c_p=ggplot(P_c)+
  geom_point(data=P_c_o%>% 
               filter(y%in%0),
             aes(x=x,y=y),
             shape=21,
             size=1.5)+
  geom_point(data=P_c_o %>% 
             ...
             fill="black",
             size=1.5)+
  geom_line(aes(x=x,y=predicted*max(ehof_med_all$`Pagrus caeruleostictus`$y)),
            color="green",
            size=1,
            linejoin="round",
            lineend="round")+
  labs(x="Depth (m)",
       y="Expected MaxN",
       title=expression(paste("C) Model III - ", italic("Pagrus caeruleostictus"))))+
  theme_classic()
P_c_p
#Model IV - Tetrosomus gibbosus
T_g=tibble(
  x=as.numeric(seq(0,150,by=1)),
  predicted=as.numeric(predict(ehof_red_all$`Tetrosomus gibbosus`,model = "IV",newdata = data.frame(x = 0:150))[,]))
#observed
T_g_o=tibble(
    x=ehof_red_all$`Tetrosomus gibbosus`$x,
    y=ehof_red_all$`Tetrosomus gibbosus`$y)
#plot
T_g_p=ggplot(T_g)+
  geom_point(data=T_g_o%>% 
               filter(y%in%0),
             aes(x=x,y=y),
             shape=21,
             size=1.5)+
  geom_point(data=T_g_o %>% 
             ...
             fill="black",
             size=1.5)+
  geom_line(aes(x=x,y=predicted*max(ehof_red_all$`Tetrosomus gibbosus`$y)),
            color="blue",
            size=1,
            linejoin="round",
            lineend="round")+
  labs(x="Depth (m)",
       y="Expected MaxN",
       title=expression(paste("D) Model IV - ", italic("Tetrosomus gibbosus"))))+
  theme_classic()
T_g_p
...
N_r=tibble(
  x=as.numeric(seq(0,150,by=1)),
  predicted=as.numeric(predict(ehof_med_all$`Nemipterus randalli`,model = "V",newdata = data.frame(x = 0:150))[,]))
#observed
N_r_o=tibble(
    x=ehof_med_all$`Nemipterus randalli`$x,
    y=ehof_med_all$`Nemipterus randalli`$y)
#plot
N_r_p=ggplot(N_r)+
  ...
  geom_point(data=N_r_o %>% 
             ...
             fill="black",
             size=1.5)+
  geom_line(aes(x=x,y=predicted*max(ehof_med_all$`Nemipterus randalli`$y)),
            color="brown",
            size=1,
            linejoin="round",
            lineend="round")+
...
ggsave(plot = HOF_panel, filename = "HOF_panel.png",device ="jpeg",height = 15,width = 25, units = "cm",dpi = 600)
```
