...
#Run eHOF
#remotes::install_version("eHOF", version = "1.12")  # Replace with your desired version

library(eHOF)
ehof_med_test=HOF(
   family=poisson,
   grad=data_bruvs_with_zeros_med %>%
     filter(species%in%"Dentex macrophthalmus") %>% 
     select(depth) %>% pull(),
...
```{r}
plot(ehof_med_test,para = T)
para_test=Para(ehof_med_test)
para_test$centralBorder
para_test$opt
```
